...
1. 默认绑定(非严格模式下this指向全局对象, 严格模式下`this`会绑定到`undefined`)

2. 隐式绑定(当函数引用有**上下文对象**时, 如 `obj.foo()`的调用方式, `foo`内的`this`指向`obj`)
...
其实这里和`1.4`很像，不过一看到函数内的函数，就很容易让人联想到闭包，然后就脱口而出，答案是`2`。

但是这里问你的是`this.a`，而在`inner`中，`this`指向的还是`window`。
...
> 简单的规则:**this 永远指向最后调用它的那个对象**。谁最后调用的函数，函数内的`this`指向的就是谁(不考虑箭头函数)。
...
有小伙伴就会有疑问了，`obj.foo()`不就相当于是`window.obj.foo()`吗？
...
既然刚刚`5`是因为函数没有返回值才报的错，那我现在给它加上返回值看看：
...
![img](./img/170b4172cf207ce5~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![img](./img/170b4177de95fd91~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
这道题是在评论区看到的，觉得挺不错的，所以加到文章中(感谢[👀](https://juejin.cn/user/2471357868148904)小伙伴提供的题目)
...
![img](./img/170b419ec8f56161~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![img](./img/170b41a39d643cbe~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
