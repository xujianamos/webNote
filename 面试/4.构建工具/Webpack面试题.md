...
5. `svg-inline-loader`：将压缩后的 SVG 内容注入js代码中。

## 1.2 样式相关loader

1. `style-loader`：把 CSS 代码注入到 JavaScript 中，通过 DOM 操作去加载 CSS。
2. `css-loader`：加载 CSS，支持模块化、压缩、文件导入等特性。
3. `sass-loader`：将SCSS/SASS代码转换成CSS。
4. `less-loader`：将less代码转换成CSS。
...
3. `web-webpack-plugin`：可方便地为单页应用输出 HTML，比 html-webpack-plugin 好用。
4. `clean-webpack-plugin`: 目录清理。

5. `terser-webpack-plugin`: 支持压缩 ES6 (Webpack4)
6. `serviceworker-webpack-plugin`：为网页应用增加离线缓存功能。
7. `speed-measure-webpack-plugin`: 可以看到每个 Loader 和 Plugin 执行耗时 (整个打包耗时、每个 Plugin 和 Loader 耗时)。
...
1. 推送更新：实际上 `WDS(Webpack-dev-server)` 与浏览器之间维护了一个 `Websocket`，当本地资源发生变化时，WDS 会向浏览器推送更新，并带上构建时的 hash，让客户端与上一次资源进行对比。

2. 拉取更新：客户端对比出差异后会向 WDS 发起 `Ajax` 请求来获取更改内容(文件列表、hash)，这样客户端就可以再借助这些信息继续向 WDS 发起 `jsonp` 请求获取该chunk的增量更新。
3. 处理资源：后续的部分(拿到增量更新之后如何处理？哪些状态该保留？哪些又需要更新？)由 `HotModulePlugin` 来完成，提供了相关 API 以供开发者针对自身场景进行处理，像`react-hot-loader` 和 `vue-loader` 都是借助这些 API 实现 HMR。
