...
- 使用`.eslintrc`  文件配置规则（支持 JSON 和 YAML 两种语法）
-  `.eslintrc `放在项目根目录，则会==应用到整个项目==；如果子目录中也包含  `.eslintrc `文件，则==子目录会忽略根目录的配置文件==，==应用该目录中的配置文件==。这样可以方便地对不同环境的代码应用不同的规则。
