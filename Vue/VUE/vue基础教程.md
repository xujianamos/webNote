...
指令是以`V-`开头的（使用指令需要`v-`开头，自定义指令时则不需要加前缀`v-`），并且都在==开始标签==中添加。
...
放在开始标签内和插值表达式搭配使用，然后给这个属性定义样式。

应用：解决插值表达式闪烁问题。

原理：没有请求到数据时，隐藏这个属性所在的标签，接收到数据时，就移除这个属性。
...
该指令表示元素和组件只渲染一次，不会随着数据的改变而改变。该指令后面不需要跟任何表达式。
...
说明：表单类型`radio`元素只有在`name`相同时，才会产生互斥。但是使用`v-model`指令也会实现互斥。
...
- 当是多个复选框时，因为可以选中多个，所以绑定的属性是一个数组。
...
Vue允许你自定义过滤器，**可被用作一些常见的文本格式化**。过滤器可以用在两个地方：插值和 `v-bind` 表达式。过滤器应该被添加在JavaScript 表达式的尾部，由管道符指示；
...
> **注**： 过滤器调用的时候，采用的是就近原则，如果私有过滤器和全局过滤器名称一致了，这时候优先调用私有过滤器
